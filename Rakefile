...
task :web_sync do
  ...
  system 'git fetch origin master:master && git checkout master && git remote add upstream https://github.com/urho3d/Urho3D.git && git fetch upstream && git merge -m "Web-sync at #{timestamp}." upstream/master && git config user.name '#{ENV['GIT_NAME']}' && git config user.email '#{ENV['GIT_EMAIL']}' && git remote set-url --push origin https://#{ENV['GH_TOKEN']}@github.com/weitjong/Urho3D.git && git push -q -u origin master >/dev/null 2>&1'
end
